...
if (serviceAccountPath) {
    ...
    console.warn('[Gateway] No Firebase Service Account found. Auth verification will fail.');
}
...
io.on('connection', (socket) => {
    console.log(`[Gateway] Client connected: ${socket.id} (User: ${socket.user?.email})`);

    socket.on('subscribe_build', (deploymentId) => {
        socket.join(`build_${deploymentId}`);
        console.log(`[Gateway] ${socket.user?.email} subscribed to build_${deploymentId}`);
        socket.emit('log', `[System] Connected to build stream for ${deploymentId}`);
    });
