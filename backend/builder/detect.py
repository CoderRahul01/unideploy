...
def detect_project_type(project_path):
    ...
    if "package.json" in files:
        with open(os.path.join(project_path, "package.json"), "r") as f:
            ...
            if "next" in all_deps:
                return {"type": "dynamic", "framework": "nextjs", "build_command": "npm run build", "start_command": "npm run start"}
            elif "vite" in all_deps:
                return {"type": "static", "framework": "vite", "build_command": "npm run build", "output_dir": "dist"}
            elif "react-scripts" in all_deps:
                return {"type": "static", "framework": "create-react-app", "build_command": "npm run build", "output_dir": "build"}
            else:
                return {"type": "dynamic", "framework": "nodejs", "start_command": "npm start"}

    # 2. Check for Python projects
    if "requirements.txt" in files or "pyproject.toml" in files:
        # Simple detection for now
        return {"type": "dynamic", "framework": "python", "start_command": "uvicorn main:app --host 0.0.0.0 --port 8080"}
