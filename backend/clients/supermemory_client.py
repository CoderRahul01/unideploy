...
class SuperMemoryClient:
    ...
    def add_context(self, content: str, source: str = "unideploy"):
        ...
        try:
            ...
            res = requests.post(f"{self.BASE_URL}/add", json=payload, headers=self.headers)
    ...
    def query(self, question: str):
        ...
        """
        if not self.api_key: return "SuperMemory not configured."
        try:
            payload = {"query": question}
            res = requests.post(f"{self.BASE_URL}/query", json=payload, headers=self.headers)
            res.raise_for_status()
