...
class StateAuthority:
    @staticmethod
    def get_effective_state(project: models.Project, k8s_client: client.ApiClient) -> str:
        """
        ...
        try:
            ...
            pods = v1.list_namespaced_pod(
                namespace="default", 
                label_selector=f"app={project.name}"
            ).items
            ...
            return project.status if project.status in ["SLEEPING", "BUILT"] else "SLEEPING"
...
class SystemGuard:
    ...
    def can_start_project(project: models.Project, db: Session) -> (bool, str):
        ...
        if project.daily_runtime_minutes >= DAILY_RUNTIME_LIMIT_MINS:
            return False, f"Daily runtime limit reached ({DAILY_RUNTIME_LIMIT_MINS}m). Resets tomorrow."

        # 2. Global Safety Limit Check
        running_pods = db.query(models.Project).filter(models.Project.status == "RUNNING").count()
        ...
        user_running = db.query(models.Project).filter(
    ...
    def can_build_project(db: Session) -> (bool, str):
        ...
        building_count = db.query(models.Deployment).filter(models.Deployment.status == "building").count()
        if building_count >= MAX_CONCURRENT_BUILDS:
            return False, "Platform build capacity reached. Please try again in a few minutes."
        return True, "OK"
    ...
    def check_invariants(project: models.Project, db: Session):
        ...
        # 1. Runtime Invariant
        assert project.daily_runtime_minutes <= DAILY_LIMIT + 5, f"CRITICAL: {project.name} exceeded daily limit significantly!"
        ...
        assert user_running <= 1, f"CRITICAL: User {project.owner_id} has multiple running projects!"
