    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                active_pod_names = [p.metadata.labels.get("app") for p in pods if p.status.phase == "Running"]
            ...
            for project in projects:
                ...
                if project.status != effective_status:
                    print(f"[Maintenance] Syncing {project.name}: {project.status} -> {effective_status}")
            ...
            projects_to_reset = db.query(models.Project).filter(
                models.Project.last_reset_at < now - datetime.timedelta(days=1)
            ).all()
